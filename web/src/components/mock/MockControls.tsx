  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-200 flex w-80 flex-col border-l border-line bg-[#0b0b10] font-sans shadow-[-18px_0_40px_rgba(0,0,0,0.45)]">
      <header className="flex items-start justify-between gap-2 border-b border-line px-3.5 pt-3.5 pb-2.5">
        ...
        <button
          type="button"
          className="border-none bg-transparent text-[14px] text-dim hover:text-body"
      ...
      <div className="flex gap-1 border-b border-line px-3.5 py-2.5">
        {(["game", "entry"] as const).map((value) => (
          <button
            ...
            type="button"
            className={cx(
              "flex-1 rounded-chip border border-line bg-surface p-1.75 text-[11px] text-dim capitalize",
              screen === value && "border-transparent bg-accent text-white",
            )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3.5 pt-2 pb-3.5 [scrollbar-color:var(--color-line)_transparent] scrollbar-thin">
        {Object.entries(groups).map(([group, items]) => (
          <section key={group} className="mt-2.5">
            <h3 className="mt-0 mb-1.5 text-[9.5px] tracking-[0.12em] text-[#6d6c80] uppercase">
              {group}
            </h3>
            {items.map((entry) => {
              ...
              return (
                <div
                  key={entry.key}
                  className={cx(
                    "mb-1 flex items-center gap-2 rounded-[9px] border border-line bg-surface px-2 py-1.5",
                ...
                >
                  <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2">
                    ...
                    <span className="flex min-w-0 flex-col">
                      ...
                      <span className="truncate text-[9.5px] text-[#6d6c80]">
                        {entry.file}
                      </span>
                    </span>
                  </label>
                  <button
                    ...
                    className={cx(
                      ...
                      flag.mock
                        ? "border-transparent bg-good text-[#0b0b0f]"
                        : "border-line bg-[#1c1c24] text-dim",
                    )}
      ...
      <footer className="border-t border-line px-3.5 py-2.5 text-[10px]">
        <span className="mb-0.75 block tracking-[0.1em] text-[#6d6c80] uppercase">
          last action
        </span>
        <code className="block max-h-12 overflow-auto break-all text-accent">
          {lastAction || "—"}
        </code>
      </footer>
    </aside>
